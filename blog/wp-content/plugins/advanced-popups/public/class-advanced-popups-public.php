...
class ADP_Public {
	...
	public function wp_footer() {
		...
		foreach ( $popups as $popup ) {
							...
							<div class="adp-popup-outer" style="<?php echo esc_attr( $outer_style ); ?>">
								<?php if ( $has_post_thumbnail ) { ?>
									<div class="adp-popup-thumbnail">
										<?php echo get_the_post_thumbnail( $popup->ID, 'csco-thumbnail', array( 'class' => 'adp-lazyload-disabled' ) ); ?>
									</div>
								<?php } ?>
			...
			if ( function_exists( 'powerkit_gutenberg' ) ) {
				// Parse blocks.
				$blocks = parse_blocks( $popup->post_content );

				$blocks = powerkit_gutenberg()->parse_blocks_css( $blocks );

				echo sprintf( '<style>%s</style>', $blocks ); // XSS.
			}
		}
	}
