...
if ( is_admin() ) {
	...
	$wpb_of_page= 'appearance_page_options-framework';
	add_action( "admin_print_styles-$wpb_of_page", 'wpb_optionsframework_mlu_css', 0 );
	add_action( "admin_print_scripts-$wpb_of_page", 'wpb_optionsframework_mlu_js', 0 );	
}
...
if ( ! function_exists( 'wpb_optionsframework_mlu_init' ) ) {
	function wpb_optionsframework_mlu_init () {
		register_post_type( 'wpb_optionsframework', array(
			'labels' => array(
...
if ( ! function_exists( 'wpb_optionsframework_mlu_css' ) ) {

	function wpb_optionsframework_mlu_css () {
...
if ( ! function_exists( 'wpb_optionsframework_mlu_js' ) ) {

	function wpb_optionsframework_mlu_js () {
	
		// Registers custom scripts for the Media Library AJAX uploader.
		wp_register_script( 'of-medialibrary-uploader', WPB_OPTIONS_FRAMEWORK_URL .'js/of-medialibrary-uploader.js', array( 'jquery', 'thickbox' ) );
 ...
 *
 * Dependencies:
 * - wpb_optionsframework_mlu_get_silentpost()
 */

if ( ! function_exists( 'wpb_optionsframework_medialibrary_uploader' ) ) {

	function wpb_optionsframework_medialibrary_uploader( $_id, $_value, $_mode = 'full', $_desc = '', $_postid = 0, $_name = '') {
		...
		$int = wpb_optionsframework_mlu_get_silentpost( $id );
		...
		if ( $_desc != '' ) {
			$output .= '<span class="wpb_of_metabox_desc">' . $_desc . '</span>' . "\n";
		}
		...
		if ( $value != '' ) { 
			...
			if ( $image ) {
				...
				$title = __( 'View File', 'wpb_optionsframework' );
 ...
 *
 * Example Usage:
 * wpb_optionsframework_mlu_get_silentpost ( 'wpb_of_logo' );
 */

if ( ! function_exists( 'wpb_optionsframework_mlu_get_silentpost' ) ) {

	function wpb_optionsframework_mlu_get_silentpost ( $_token ) {
	
		global $wpdb;
		$_id = 0;
	
		// Check if the token is valid against a whitelist.
		// $_whitelist = array( 'wpb_of_logo', 'wpb_of_custom_favicon', 'wpb_of_ad_top_image' );
		// Sanitise the token.
		...
		if ( $_token ) {
			...
			$_args = array( 'post_type' => 'wpb_optionsframework', 'post_name' => 'of-' . $_token, 'post_status' => 'draft', 'comment_status' => 'closed', 'ping_status' => 'closed' );
...
if ( ! function_exists( 'wpb_optionsframework_mlu_insidepopup' ) ) {

	function wpb_optionsframework_mlu_insidepopup () {
	
		if ( isset( $_REQUEST['is_optionsframework'] ) && $_REQUEST['is_optionsframework'] == 'yes' ) {
		
			add_action( 'admin_head', 'wpb_optionsframework_mlu_js_popup' );
			add_filter( 'media_upload_tabs', 'wpb_optionsframework_mlu_modify_tabs' );
		}
	}
}

if ( ! function_exists( 'wpb_optionsframework_mlu_js_popup' ) ) {

	function wpb_optionsframework_mlu_js_popup () {

		$_wpb_of_title = $_REQUEST['wpb_of_title'];
		if ( ! $_wpb_of_title ) { $_wpb_of_title = 'file'; } // End IF Statement
?>
	...
	jQuery(function($) {
		...
		$( 'h3.media-title' ).each ( function () {
			...
			var new_title = current_title.replace( 'media file', '<?php echo $_wpb_of_title; ?>' );
			$( this ).html( new_title );
...
if ( ! function_exists( 'wpb_optionsframework_mlu_modify_tabs' ) ) {

	function wpb_optionsframework_mlu_modify_tabs ( $tabs ) {
		$tabs['gallery'] = str_replace( __( 'Gallery', 'wpb_optionsframework' ), __( 'Previously Uploaded', 'wpb_optionsframework' ), $tabs['gallery'] );
		return $tabs;
	}
}