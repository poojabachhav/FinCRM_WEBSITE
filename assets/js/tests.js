...
test('Pauses on click when allowPause is true', function(assert) {
  ...
  setTimeout(function() {
    assert.equal(timerElement.text(), '00:03', 'Timer is on pause');
    done();
  }, 3000);
});
...
test('Does NOT pause on click when allowPause is not specified', function(assert) {
  ...
  setTimeout(function() {
    assert.equal(timerElement.text(), '00:01', 'Cleared timer');
    done();
  }, 1500);
});
...
test('Does NOT pause on click when allowPause is false', function(assert) {
  ...
  setTimeout(function() {
    assert.equal(timerElement.text(), '00:00', 'Cleared timer');
    done();
  }, 3000);
});
